    ...
    private function buildChartHTMLRGraph(array $reportData, array $fields, AOR_Field $mainGroupField = null) {
        ...
        switch ($this->type) {
            ...
            case 'grouped_bar':
                ...
                $chart = $this->getRGraphGroupedBarChart(json_encode($data['data']), json_encode($data['labels']), json_encode($data['tooltips']), $this->name, $this->id, $defaultHeight, $defaultWidth, true, $xName, $yName,json_encode($data['columns']));
                break;
            case 'stacked_bar':
                $chartFunction = 'Stacked bar';
                $data = $this->getRGraphGroupedBarChartData($reportData, $xName, $yName, $mainGroupField);
                $config = $this->getStackedBarChartConfig();
                $chart = $this->getRGraphGroupedBarChart(json_encode($data['data']), json_encode($data['labels']), json_encode($data['tooltips']), $this->name, $this->id, $defaultHeight, $defaultWidth, false, $xName, $yName,json_encode($data['columns']));
                break;
            case 'bar':
            default:
                $chartFunction = 'Bar';
...
EOF;
    ...
    //or stacked (false)
    private function getRGraphGroupedBarChart($chartDataValues, $chartLabelValues, $chartTooltips, $chartName, $chartId, $chartHeight = 400, $chartWidth = 400, $grouped = false, $xName, $yName,$allcolumns) {
        $dataArray = json_decode($chartDataValues);
...
//        $tooltips[] = preg_replace('/[0-9]*$/', '', $xName);
////        $tooltips[] = preg_replace ( '/[0-9]*$/' , '' , $yName);;
//        foreach ($get_tooltips as $t) {
//            $tooltips[] = trim(strstr($t, '(', true));
//        }
//        $tooltips = array_diff($tooltips, array(''));
       
        $tooltips = (array) json_decode($allcolumns);
        array_unshift($tooltips, preg_replace('/[0-9]*$/', '', $xName));
        $unique_tooltips = "'" . implode("','", $tooltips) . "'";
   
        $max_count[] = (count(array_unique($tooltips)) - 1);                  //Creating maximum columns for the grouped and stack chart
...
drawStuffBarGroupedStacked$isStackedChart();
});

        function drawStuffBarGroupedStacked$isStackedChart() {
        ...
        var chart = new google.visualization.ColumnChart(document.getElementById('chart_div$isStackedChart$chartId'));
        ...
        google.load('visualization', '1', {packages:['corechart'], callback: drawStuffBarGroupedStacked$isStackedChart});
        });
        </script>
        <div id="chart_div$isStackedChart$chartId" class="google_chart"></div><br>
...
EOF;
    ...
    private function getRGraphGroupedBarChartData($reportData, $xName, $yName, AOR_Field $mainGroupField = null) {
        ...
        $_labels = array();
        $_tooltips = array();
        //Stack and grouped chart columns mismatch issue by Roshan Sarode dt. 1-10-18 start
        $_only_tooltips = array();


        foreach ($data as $flabel => $fvalues) {

            foreach ($fvalues as $ftooltip => $fvalue) {
                $_only_tooltips[] = $ftooltip;
            }
        }

        $only_tooltips = array_unique($_only_tooltips);
       
        foreach ($data as $label => $values) {
            $_labels[] = $this->getShortenedLabel($label);
            $_gettooltips = array();
            foreach ($values as $tooltip => $value) {
                $_gettooltips[] = $tooltip;
            }
            $result = array_diff($only_tooltips, $_gettooltips);

            $_values = array();

            foreach ($only_tooltips as $key_tooltip) {
                if (array_key_exists($key_tooltip, $values)) {
                    $_tooltips[] = $key_tooltip . " ($values[$key_tooltip])";
                    $_values[] = $values[$key_tooltip];
                } else {
                    $_tooltips[] = $key_tooltip . " (0)";
                    $_values[] = 0;
                }
            }
        ...
        //Stack and grouped chart columns mismatch issue by Roshan Sarode dt. 1-10-18 end
