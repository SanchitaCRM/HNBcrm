    ...
    function build_report_chart($chartIds = null, $chartType = self::CHART_TYPE_PCHART) {
        ...
        switch ($chartType) {
            ...
            case self::CHART_TYPE_RGRAPH:
                /* To check Rgraph
                  Modified By Swapnil 2 June2017
                 */
...
//                }
                if ($_REQUEST['module'] == 'AOR_Reports') {//Need the require_once for the rgraphincludes as they are only loaded when the home page is hit
                    require_once('include/SuiteGraphs/RGraphIncludes.php');
                }
                /* To check Rgraph End */
    ...
    function build_group_report($offset = -1, $links = true, $extra = array(), $subgroup = '') {
        ...
        if ($field_id != '' && empty($subgroup)) {
            ...
            if ((isset($data['source']) && $data['source'] == 'custom_fields')) {
                ...
                $query_array = $this->build_report_query_join($table_alias . '_cstm', $table_alias . '_cstm', $table_alias, $field_module, 'custom', $query_array);
            } else {
            ...
            /* When the report is grouped with main group, it displayes same tables multiple times. To restrict that, following changes are made by Shakeer on 31st Jan 2018 
             * For some specific reports Same table showing multiple times
             * Dt. 23-11-2018
             * By Roshan Sarode start
             */

            $row_list = array();
            while ($row = $this->db->fetchByAssoc($result)) {

                if (!in_array(trim(strtolower($row[$field_label])), $row_list)) {
                    if ($html != '') {
                        $html .= '<br />';
                    }
                    ...
                    $row_list[] = strtolower($row[$field_label]);
                    }
                }
            }
            /*
             * For some specific reports Same table showing multiple times
             * Dt. 23-11-2018
             * By Roshan Sarode end
             */
            //End
        }
        if ($html == '') {
            /*
             * Pagination not working in table issue 
             * Changed from $html = $this->build_report_html($offset, $links, '', create_guid(), $extra);
             * Changed to  $html = $this->build_report_html($offset, $links, $subgroup, create_guid(), $extra);
             * Dt. 01-12-2018
             * By Roshan Sarode Start
             */
            $html = $this->build_report_html($offset, $links, $subgroup, create_guid(), $extra);
            /*
             * Pagination not working in table issue e
             * Changed from $html = $this->build_report_html($offset, $links, '', create_guid(), $extra);
             * Changed to  $html = $this->build_report_html($offset, $links, $subgroup, create_guid(), $extra);
             * Dt. 01-12-2018
             * By Roshan Sarode end
    ...
    function build_report_html($offset = -1, $links = true, $group_value = '', $tableIdentifier = '', $extra = array()) {
        ...
        fwrite($myfile, print_r($report_sql, true));
        fclose($myfile);
         ...
         * Dt. 04-12-2018 Start
         */
        $table_group = str_replace(' ', '', $group_value);
        $table_group = preg_replace('/[^A-Za-z0-9\-]/', '', $table_group);

        $html.='<table id="report_table_' . $tableIdentifier . $table_group . '" cellpadding="0" cellspacing="0" width="100%" border="0" class="list view table-responsive aor_reports">';
        /*
         * (Bug:1001)Pagination not working because of in html table id due to group value special characters and spaces are coming
         * Changes by Roshan Sarode
         * Dt. 04-12-2018 End
        ...
        while ($row = $this->db->fetchByAssoc($result)) {
            ...
            if ($fields[$label]['display']) {
                ...
                // End
                $html .= "<div style='white-space: normal;text-transform:uppercase;font-weight:bolder;' width='100%' align='left'>";
                $html .= $field->label;
                $html .= "</div></th>";
            }
            ++$i;
...
//                </button>";
                $html .="
            <button class='btn pagination-icon-bottom-disabled btn-outline-primary btnbackcolor' id='listViewPrevButton_top' name='listViewPrevButton' title='Previous' disabled='disabled' style='color:#6d6d6d'>
...
//                </button>";
                $html .="
            <button  id='listViewPrevButton_top' class='btn pagination-icon-bottom btn-outline-primary btnbackcolor' name='listViewPrevButton' title='Previous' onclick='changeReportPage(\"" . $this->id . "\"," . $previous_offset . ",\"" . $group_value . "\",\"" . $tableIdentifier . "\")'>
            <i class='fa fa-caret-left fa-lg ' aria-hidden='true'></i>
            </button>&nbsp;&nbsp;&nbsp;&nbsp;";
            }
...
//                    </button>";
                $html .="&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<button  id='listViewNextButton_top' class='btn pagination-icon-bottom btn-outline-primary btnbackcolor' name='listViewNextButton' title='Next' onclick='changeReportPage(\"" . $this->id . "\"," . $next_offset . ",\"" . $group_value . "\",\"" . $tableIdentifier . "\")'>
...
//                       <span class='suitepicon suitepicon-action-last'></span>
//                    </button>";
                $html .="&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<button id='listViewNextButton_top' class='btn pagination-icon-bottom-disabled btn-outline-primary btnbackcolor' name='listViewNextButton' title='Next'   disabled='disabled' style='color:#6d6d6d'><i class='fa fa-caret-right fa-lg ' aria-hidden='true'></i>
            </button>&nbsp;&nbsp;";
            }
        ...
        while ($fieldCount && $row = $this->db->fetchByAssoc($result)) {
            ...
            foreach ($fields as $name => $att) {
                if ($att['display']) {
                    ...
                    if ($att['total']) {
                        if ($currency_id != '') {
                            $totals[$name][] = getModuleField($att['module'], $att['field'], $att['field'], 'DetailView', $row[$name], '', $currency_id, $att['params']);
                        } else {
    ...
    function getTotalHTML($fields, $totals) {
        ...
        /* Modified by Swapnil */
        $get_report_total = "SELECT total from aor_fields where aor_report_id='$report_id' and total ='SUM' and deleted=0";
        $result_report_total = $this->db->query($get_report_total);
        $row_report_total = $this->db->fetchByAssoc($result_report_total);
        $total = $row_report_total['total'];
        /* Modified by Swapnil */
...
//        $html .= "<tbody>";
                        ...
                        switch ($type) {
                            ...
                            case 'AVG':
                                if ($currency->id == -99) {
                                    ...
                                    $total = $currency->symbol . format_number($total, null, null); // modified by swapnil for currency format
                                }
                                break;
                            case 'COUNT':
                            default:
    ...
    function calculateTotal($type, $totals) {
        ...
        $numb_sep = $num_grp_sep = $sugar_config['default_number_grouping_seperator'];
        switch ($type) {
            case 'SUM':
                $totals = str_replace($currency_symbol, '', $totals);
                $totals = str_replace($numb_sep, '', $totals);
                //echo ">>>>".print_r(array_sum($totals));exit;
    ...
    function build_report_query($group_value = '', $extra = array()) {
        ...
        /*
         * Group by condition was working differently in html table and chart, due to that issue condition added for checking is html table or chart 
         * Changes by Roshan Sarode
         * Dt. 26-11-2018 start
         */
        $query_array['istable'] = $extra['istable'];
        /*
         * Group by condition was working differently in html table and chart, due to that issue condition added for checking is html table or chart 
         * Changes by Roshan Sarode
         * Dt. 26-11-2018 End
            ...
            while ($row = $this->db->fetchByAssoc($result)) {
                ...
                if (!empty($path[0]) && $path[0] != $module->module_dir) {
                    foreach ($path as $rel) {
                        ...
                        $query = $this->build_report_query_join(
                                $rel, $table_alias, $oldAlias, $field_module, 'relationship', $query, $new_field_module);
                        $field_module = $new_field_module;
                    }
                }
                $data = $field_module->field_defs[$field->field];
                ...
                if ($data['type'] == 'currency' && isset($field_module->field_defs['currency_id'])) {
                    if ((isset($field_module->field_defs['currency_id']['source']) && $field_module->field_defs['currency_id']['source'] == 'custom_fields')) {
                        ...
                        /* second_group_by option generating wrong query due to this reason it's commented
                         * Changes By Roshan Sarode
                         * 05-11-2018 Start
                         */
...
//                        $query['second_group_by'][] = $this->db->quoteIdentifier($table_alias) . ".currency_id";
                ...
                if ($field->link && isset($query['id_select'][$table_alias])) {
                    ...
                    //Issue : After adding link and group option for any field, "Group " option not working properly (secondary grouping issue) by roshan dt. 25-9-18
                    if ($field->group_display == 1 && empty($query['istable'])) {
                ...
                if ($field->group_by == 1) {
                     ...
                     * 05-11-2018 Start
                     */
                    if ($field->group_display == 1 && !isset($query['istable'])) {
                 ...
                 * Dt. 26-11-2018 start
                 */
//                if ($field->group_display == 1 && empty($group_value) && $query['istable'] == 1) {
//                    $query['where'][0] = '(' . $select_field . " = '' OR " . $select_field . ' IS NULL) AND ';
//                } else
...
//echo "</pre>";
    ...
    $name, $alias, $parentAlias, SugarBean $module, $type, $query = array(), SugarBean $rel_module = null
    ) {
                ...
                if (isset($aor_sql_operator_list[$condition->operator])) {
                    ...
                    switch ($condition->value_type) {
                        ...
                        case "Period":
                            ...
                            $periodparams = array();
                            $periodparams[] = $params;
                            $periodparams[] = $condition->operator;
                            $value = '"' . getPeriodDate($periodparams)->format('Y-m-d H:i:s') . '"';
                            /**
                             * Period Greater than and Less than or equal to not showing proper result 
                             * Changes by Roshan Sarode
                             * Dt. 03-11-2018   End
