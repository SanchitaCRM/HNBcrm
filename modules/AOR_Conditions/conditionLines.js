...
//Report module: On change of report module fields not appearing in left side tree view fixed by Roshan Sarode dt. 08-10-2018 start
$(document).ready(function () {
    $("#report_module").on('change', function () {
        clickOnModuleTreeValue();
    })
    clickOnModuleTreeValue();
})
function clickOnModuleTreeValue() {
        var mod_report = $("#report_module option:selected").text();
        setTimeout(function () {
                $('.jqtree-title-folder').each(function (index, value) {

                if ($(this).html() == mod_report) {
                    span = $(this);
                    $(span).trigger("click");
                }
            });
...
//Report module: On change of report module fields not appearing in left side tree view fixed by Roshan Sarode dt. 08-10-2018 end
var LogicalOperatorHandler = {
    ...
    getLogicalOperatorSelectHTML: function (value, _condln, forcedValue) {
        ...
        // default set to 'AND'!!
        if (typeof forcedValue == 'undefined' || forcedValue === true)
            forcedValue = 'AND';
        if (!forcedValue)
            forcedValue = null;
...
var ParenthesisHandler = {
    ...
    getParenthesisEndHtml: function (condition_id, condition_order, condition_parenthesis_start, _condln, _start_condln) {
        ...
        if (!condition_order)
            condition_order = '';
        if (!condition_parenthesis_start || condition_parenthesis_start == 'END')
            condition_parenthesis_start = '';
        if (_condln === 0)
            _condln = '0';
        if (_start_condln === 0)
            _start_condln = '0';
...
function insertConditionLine(condition) {
    ...
    if (view == 'EditView' && condition.parenthesis) {
        ...
        if (view == 'EditView') {
            ...
            cellLogic.style.width = '15%';
            cellLogic.style.color = "rgb(0,0,0)";
