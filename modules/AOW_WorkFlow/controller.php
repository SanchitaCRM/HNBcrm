...
class AOW_WorkFlowController extends SugarController {
    ...
    protected function action_getSMSField() {
        $module = $_REQUEST['aow_module'];
        $aow_field = $_REQUEST['aow_newfieldname'];
        $aow_phone_field = $_REQUEST['aow_newfieldphonetypename'];
        $ln = $_REQUEST['ln'];
        $cln = $_REQUEST['cln'];
        ...
        switch ($_REQUEST['aow_type']) {
            case 'Record Phone';
                ...
                foreach ($field_defs[$module] as $fn) {
                    ...
                    if (!empty($get_lable) && $fn['type'] == 'phone') {
                        $phone_field_list[$fn['name']] = str_replace(":", "", $get_lable);
                    }
                }
            ...
            case 'Related Field':
                $rel_field_list = getRelatedEmailableFields($module);
                $rel_field_list2=$rel_field_list;
                reset($rel_field_list2);
$first_key = key($rel_field_list2);
        ...
        $key = !empty($_REQUEST['aow_value'])?$_REQUEST['aow_value']:$first_key;
        $smsTarget = $key;
        ...
        $module1 = $field['module'];
                $bean1 = BeanFactory::getBean($module1);
                $field_defs[$module1] = $bean1->getFieldDefinitions();
                foreach ($field_defs[$module1] as $fn) {
                    if (file_exists('modules/' . $module1 . '/language/en_us.lang.php')) {
                        include('modules/' . $module1 . '/language/en_us.lang.php');
                    } else {
                        include('custom/modules/' . $module1 . '/language/en_us.lang.php');
                    }
                    $get_lable = $mod_strings[$fn['vname']];
                ...
                if ($view == 'EditView') {
                    echo "<select type='text'  name='$aow_field' id='$aow_field' title='' tabindex='116' onchange='show_relModuleField(this,$ln,$cln);'>" . get_select_options_with_id($rel_field_list, $value) . "</select>";
                } else {
                    echo $rel_field_list[$value];
                }
                break;
            case 'Specify User':
                ...
                $field_defs['Users'] = $bean->getFieldDefinitions();
                foreach ($field_defs['Users'] as $fn) {
                    if (file_exists('modules/Users/language/en_us.lang.php')) {
                        include('modules/Users/language/en_us.lang.php');
                    } else {
                        include('custom/modules/Users/language/en_us.lang.php');
                    }
                    $get_lable = $mod_strings[$fn['vname']];
        ...
        if ($_REQUEST['aow_type'] == 'Related Field' || $_REQUEST['aow_type'] == 'Specify User' || $_REQUEST['aow_type'] == 'Users' || $_REQUEST['aow_type'] == 'Record Phone') {

            if ($view == 'EditView') {
                echo "<select type='text'  name='$aow_phone_field' id='$aow_phone_field' title='' tabindex='116'>" . get_select_options_with_id($phone_field_list, $phone_field) . "</select>";
            } else {
                echo $rel_field_list[$value];
            }
        }
    ...
    protected function action_getRelPhoneField() {
        ...
        $field_defs[$module] = $bean2->getFieldDefinitions();
        foreach ($field_defs[$module] as $fn) {
            if (file_exists('modules/' . $module . '/language/en_us.lang.php')) {
                include('modules/' . $module . '/language/en_us.lang.php');
            } else {
            ...
            if (!empty($get_lable) && $fn['type'] == 'phone') {
                $phone_field_list .= "<option value=".$fn['name'].">".str_replace(":", "", $get_lable)."</option>";
            }
        }
       echo $phone_field_list;exit;
