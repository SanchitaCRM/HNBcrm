...
$form_title, $other_text, $show_help,
 /* Module Icons changes made by Roshan Sarode */ $module_nm = NULL
) {
    ...
    if ($is_min_max !== false)
    $form_title_subpanel = $form_title;
   ...
   /* Module Icons changes made by Roshan Sarode */
    ...
    $cicon = (!empty($icon_module[$module_nm]['faicon'])) ? $icon_module[$module_nm]['faicon'] : $icon_module['default']['faicon'];
    $bicon = (!empty($icon_module[$module_nm]['bgcolor'])) ? $icon_module[$module_nm]['bgcolor'] : $icon_module['default']['bgcolor'];
    ...
    $form_title_subpanel = "{$other_text}&nbsp;$icon {$form_title_subpanel}";
 
    if ($_REQUEST['action'] != "DetailView" && $_REQUEST['action'] != "TrackDetailView") {

        $dashlettitle = substr($form_title, 0, 30) . ((strlen($form_title) > 30) ? '...' : '');
        $the_form .= <<<EOHTML

<table width="100%" cellpadding="0" cellspacing="0" border="0" class="formHeader h3Row ">
<tr>
<td nowrap><h3><span class="change_color_dashlets" title="{$form_title}">$icon {$dashlettitle}</span></h3></td>
EOHTML;
    } else {

        $the_form .= <<<EOHTML
<div nowrap><span class="change_color_dashlets" style="font-size:14px"><a href="javascript:void(0)">{$form_title_subpanel}</a></span></div>
EOHTML;
    }
...
$module, $module_title, $show_create, $count = 0
) {
