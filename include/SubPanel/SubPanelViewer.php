...
/*
 * After adding and removing record subpanel count not updating issue resolved
 * By Roshan Sarode
 * Dt: 19-12-2018 Start
 */
...
echo $js = <<<JS
        <script>
        var subp = "$subpanel";
        var get_subp= "list_subpanel_"+subp;
        var getpageNumbers=$("#"+get_subp+" .pageNumbers").html();
        tot_record=(getpageNumbers.split("of")[1]).trim().replace(/[`~!@#$%^&*()_|+\-=?;:'",.<>\{\}\[\]\\\/]/gi, '');
        $("#"+subp+"_CELL").attr('data-record', tot_record);
        $("#"+subp+"_CELL").html("(" + tot_record + ")");
