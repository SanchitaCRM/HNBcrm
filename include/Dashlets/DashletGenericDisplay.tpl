...
<div class="table-responsive">
    ...
    <table id="dashletPanel" cellpadding='0' cellspacing='0' width='100%' border='0' class='list view default'>
        <thead>
                        ...
                        <tr>
                            ...
                            <td align='right' nowrap='nowrap'>
                                ...
                                {if $pageData.urls.prevPage}
                                    ...
                                    &nbsp;&nbsp;
                                    <button id="" class="btn pagination-icon-bottom btn-outline-primary btnbackcolor" onclick='return SUGAR.mySugar.retrieveDashlet("{$dashletId}", "{$pageData.urls.prevPage}")' style="cursor: pointer;" title="{$navStrings.previous}">
                                ...
                                {else}
                                    ...
                                    &nbsp;&nbsp;
                                    <button id="" class="btn pagination-icon-bottom-disabled btn-outline-primary btnbackcolor" disabled='disabled' style="color:#6d6d6d">
                                      <i class="fa fa-caret-left fa-lg" aria-hidden="true"></i>
                                    </button> &nbsp;&nbsp;
                                ...
                                {if $pageData.urls.nextPage}
                                    ...
                                    &nbsp;&nbsp;
                                    <button id="" class="btn pagination-icon-bottom btn-outline-primary btnbackcolor" onclick='return SUGAR.mySugar.retrieveDashlet("{$dashletId}", "{$pageData.urls.nextPage}")' style="cursor: pointer;" title="{$navStrings.next}">
                                ...
                                {else}
                                   <!-- &nbsp;{$navStrings.next}&nbsp;{sugar_getimage name="next_off" ext=".png" width="8" height="13" alt=$navStrings.next other_attributes='align="absmiddle" border="0" '}-->
                                    ...
                                    &nbsp;&nbsp;
                                    <button id="" disabled='disabled'  class="btn pagination-icon-bottom-disabled btn-outline-primary btnbackcolor" >
                                        <i class="fa fa-caret-right fa-lg" aria-hidden="true"></i>
                                    </button>
                                    &nbsp;&nbsp;
                                {/if}
            ...
            <tr height='20' class="text-uppercase">
...
{foreach name=rowIteration from=$data key=id item=rowData}
    ...
    <tr height='20' class='{$_rowColor}S1'>
        ...
        {foreach from=$displayColumns key=col item=params}
            {strip}
                <td scope='row' align='{$params.align|default:'left'}' valign="top" class="{if ($params.type == 'teamset')}nowrap{/if} {if $params.dynamic_module}{$rowData[$params.dynamic_module]}{else}{$params.module|default:$pageData.bean.moduleDir}{/if}_{$col|lower}_custom" field="{$col|lower}" >
                {if $col == 'NAME' || $params.bold}{*<b>*}{/if}
                    {if $params.link && !$params.customCode}
                        <{$pageData.tag.$id[$params.ACLTag]|default:$pageData.tag.$id.MAIN} href='index.php?action={$params.action|default:'DetailView'}&module={if $params.dynamic_module}{$rowData[$params.dynamic_module]}{else}{$params.module|default:$pageData.bean.moduleDir}{/if}&record={$rowData[$params.id]|default:$rowData.ID}&offset={$pageData.offsets.current+$smarty.foreach.rowIteration.iteration}&stamp={$pageData.stamp}' class="flip_open" onclick="flipOpen($(this));"
                        data-stuff='action:{$params.action|default:'DetailView'},module:{if $params.dynamic_module}{$rowData[$params.dynamic_module]}{else}{$params.module|default:$pageData.bean.moduleDir}{/if},record:{$rowData[$params.id]|default:$rowData.ID},offset:{$pageData.offsets.current+$smarty.foreach.rowIteration.iteration},stamp:{$pageData.stamp}' >
                    {/if}
                    {if $params.customCode}
                        {sugar_evalcolumn_old var=$params.customCode rowData=$rowData}
                    {else}
                                        {include file="include/Dashlets/FieldBackground.tpl"}
