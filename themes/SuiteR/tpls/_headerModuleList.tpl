...
//Changes for set footer on bottom -Roshan Sarode
        ...
        function changeFooterView(x) {
            ...
            var customleftpanel = $('.custom-left-panel').height();
            var heightfor = (x == 1) ? '#pageContainer' : '#bootstrap-container';

            var con = doc  - (foot + nav + listViewEmpty);
             if(page < customleftpanel){ 
                con = customleftpanel + page+10;
            }
          //  alert(con+"------>"+doc+"->>>>"+foot+"<><><>>"+nav+"<><><>"+page+">>>>>"+listViewEmpty+">>>>>>>>>>"+customleftpanel+">>>>>>>>>");
                            ...
                            <ul class="dropdown-menu" role="menu">
                                <div class="bigmenu">
                                    {foreach from=$moduleExtraMenu item=module key=name name=moduleList}
                                        ...
                                        {assign var=first1 value=$module|substr:0:1}

                                        {* sugar_link id="moduleTab_$name" module=$name data=$module *}

                                        {*Module Icons changes made by Roshan Sarode*}
                                        {php}
global $db;
...
//echo $icon_module[$module_nm]['faicon'];
                                        {/php} 
                                            ...
                                            <li>
                                                <a href="index.php?module={$name}&amp;action=index" id="moduleTab_{$name}" module="{$name}">
                                                    <span class="fa-stack">
                                                        <i class="fa fa-square-o"></i>
                                                    <i class="fa {php}echo (!empty($icon_module[$module_nm]['faicon']))? $icon_module[$module_nm]['faicon']
            :$icon_module['default']['faicon']; {/php} fa-stack-1x" style="background-color:{php}echo (!empty($icon_module[$module_nm]['bgcolor']))? $icon_module[$module_nm]['bgcolor']
            :$icon_module['default']['bgcolor']; {/php};color:#FFFFFF;border-radius:5px"></i>
                                                </span>&nbsp;&nbsp;<span title='{$module}'>{$module|truncate:20:'...':true}</span></a>
                                            </li>
	...
	global $current_user;
            ...
            <div id="globalLinks" class="dropdown navbar-nav navbar-right" style="padding:6px">
                ...
                <div class="admin-mobile">
                    ...
                    {if file_exists($profile_path_tpl)}
                        <a href='index.php?module=Users&action=DetailView&record={$CURRENT_USER_ID}' id="usermenucollapsed" style="padding-top:9px" class="profile-icon-hd dropdown-toggle"  aria-expanded="true"><div class="profile_pic" style='background-image:url({php}echo $profile_path_img;{/php})'>
                            </div>
                        </a>
                    {else}
                        {*<span class="profile_pic" style="margin-top:3px">*}
                        <a href='index.php?module=Users&action=DetailView&record={$CURRENT_USER_ID}' style="padding-top:9px" data-letters="{$FIRST_LETTER_CURRENT_USER}" id="usermenucollapsed" class="profile-icon-hd dropdown-toggle profile_pic" aria-expanded="true">{*<i class="fa fa-user-circle fa-2x" aria-hidden="true" ></i>{$CURRENT_USER}<span class="caret"></span>*}
                        </a>
                        {*</span>*}
                    {/if}    
