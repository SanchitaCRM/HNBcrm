...
{if $AUTHENTICATED}
    ...
    <div class="generic-footer">
        ...
        <div class="footer-logo-copyrights">
            <div class="footer-logo">
                <img src="custom/include/images/simplecrm.png" />
                <span class="simplecrm-version">v2.6.3</span>
            </div>
            <div class="footer-copyrights">
                <a id="powered_by" class="powered_by">&copy; {$MOD.LBL_SUITE_POWERED_BY}</a>
                <br>
                <a id="supercharged_by" class="supercharged_by" >&copy; Supercharged By SuiteCRM</a>
            </div>
        </div>
    </div>
    <!-- END Generic Footer -->
...
{literal}
    <script>
        ...
        //For appending footer at the last in body by Roshan Sarode 28-5-18
        $('.generic-footer').appendTo(document.body);
        var myTesting = "{/literal}{$smarty.request.module}{literal}";

        if (myTesting == 'AOR_Scheduled_Reports') {
            $(function () {
                $('#schedule_raw').on('change', function () {
                    if ($(this).is(':checked'))
                        $('#schedule_raw').click();
                    $('#schedule_raw').parent('label').hide();
                }).trigger('change');
            })
        }
